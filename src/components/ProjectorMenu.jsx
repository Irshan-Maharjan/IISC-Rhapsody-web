...
const ProjectorMenu = ({ isFooterVisible }) => {
    ...
    return (
        <>
            <div className={`fixed top-0 left-0 w-full z-[110] pointer-events-none transition-all duration-700 ease-in-out ${isFooterVisible ? 'opacity-0 -translate-y-20' : 'opacity-100 translate-y-0'}`}>
                <a
                    href="/"
                    ...
                    <div className="absolute inset-0 z-0">
                        ...
                        >
                            <Suspense fallback={null}>
                                <DiscoBall scale={isMobile ? 2.8 : 3.5} />
                    ...
                    <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none mix-blend-overlay">
                        <div className="relative w-8 h-8">
                            <span className={`absolute inset-0 transition-all duration-300 transform ${isOpen ? 'rotate-90 opacity-0' : 'rotate-0 opacity-100'}`}>
                                <Menu size={32} color="white" strokeWidth={3} />
                            </span>
            ...
            >
                <div className={`h-full w-full flex flex-col items-start pl-8 md:pl-20 justify-start pt-[62vh] md:pt-[52vh] transition-all duration-500 delay-200 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                    <nav className="flex flex-col gap-4 text-white text-left z-10 drop-shadow-md">
                        {['Home', 'About', 'Pronites', 'Events', 'Ambassador', 'Sponsors', 'Sponsor Us'].map((item, index) => (
                            <a
                                key={item}
                                href={`#${item.toLowerCase() === 'ambassador' ? 'campus-ambassador' : item.toLowerCase().replace(' ', '-')}`}
                                onClick={handleToggle}
